...
void main()
{
   vec3 ligthVector = normalize(vec3( 0.0f, 0.0f, -1.0f ));
   ...
   float normalx = clamp( 400.0f * fade_const * ( ((color0left.r - color0.r) - (color0.r - color0right.r)) ) * 0.5f, -1.0f, 1.0f );
   float normaly = clamp( 400.0f * fade_const * ( ((color0top.r - color0.r) - (color0.r - color0bottom.r)) ) * 0.5f, -1.0f, 1.0f );
   
   vec3 colorlightVector = vec3( normalx, normaly, 1.0-sqrt(normalx*normalx+normaly*normaly) );
   float colorLuminance = clamp( fade_const*1.0f*abs(dot( colorlightVector, ligthVector )), 0.0f, 1.0f );
   colorlight = vec4( colorLuminance, colorLuminance, colorLuminance, 1.0f );
   //colorlight = vec4( normalx, normaly, 0.0f, 1.0f );
