...
moMoldeoObject* moDirectorConsole::GetObjectByIdx( int p_object_id ) {
  moMoldeoObject* returnMOB = NULL;
  //if ( p_object_id )
  int object_id_to_index =  p_object_id - MO_MOLDEOOBJECTS_OFFSET_ID;

  if (0<=object_id_to_index && object_id_to_index<m_MoldeoObjects.Count()) {
  ...
  if (0<=object_id_to_subscene_index && object_id_to_subscene_index<m_MoldeoSceneObjects.Count()) {
...
moDirectorConsole::GetObject( moMobDescriptor p_MobDesc ) {
    ...
    switch( p_MobDesc.GetMobDefinition().GetType() ) {
        ...
        case MO_OBJECT_MASTEREFFECT:
          if (oid) {
            pMOB = (moMoldeoObject*) GetObjectByIdx(oid);
          }
          break;
...
moMobDescriptors moDirectorConsole::GetMobDescriptors() {
    ...
    for( int i=0;  i < (int) m_MoldeoSceneObjects.Count(); i++) {
        moMoldeoObject* pMOB = m_MoldeoSceneObjects.GetRef(i);
        if (pMOB) {
            moEffect* pEffect = NULL;
            switch(pMOB->GetType()) {
                case MO_OBJECT_IODEVICE:
...
moDirectorStatus moDirectorConsole::AddChildMob( moMobDescriptor p_MobDesc, moMobDescriptor p_MobDescFather ) {

  moMoldeoObject* pFatherMOB = GetObject( p_MobDescFather );
  ...
  if ( pFatherMOB ) {
    p_MobDesc.GetMobDefinition().SetMoldeoFatherId( pFatherMOB->GetMobDefinition().GetMoldeoId() );
  } else {
    //p_MobDesc.GetMobDefinition().SetMoldeoFatherId( pFatherMOB->GetMobDefinition().GetMoldeoId() );
    return MO_DIRECTOR_STATUS_ERROR;
  }

  return NewMob( p_MobDesc );
...
moDirectorConsole::EditMob( moMobDescriptor p_MobDesc ) {
      ...
      if (res==MO_DIRECTOR_STATUS_CONFIG_ALREADY_OPENED) {
        return MO_DIRECTOR_STATUS_OK;
      } else
      if (pConfig) {
...
moDirectorConsole::OnTimer( wxTimerEvent &event) {
  ...
  if (m_timer.IsRunning() && m_bInitialized) {
    ConsoleLoop();
  }
...
moDirectorStatus moDirectorConsole::Render() {
  ///return m_pDirectorConsole->SaveSession();
  MODebug2->Message("Render");
...
moDirectorStatus moDirectorConsole::ConsoleLoop() {
  ...
  if ( previewreset && ( moGetTicks() / 1000) > 1) {
      ...
      if (m_pResourceManager) {
          if (!m_pResourceManager->Initialized())
              m_pResourceManager->Init( moText(""), moText(MOLDEODATADIR "/test") , dummy, 0, 800, 600, 800, 600 );
