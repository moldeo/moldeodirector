...
BEGIN_EVENT_TABLE(wxTreeMultiCtrl, wxScrolledWindow)
  ...
  EVT_SCROLLWIN(wxTreeMultiCtrl::OnScroll)
...
wxTreeMultiItem wxTreeMultiCtrl::AddRoot(const wxString &caption, const wxString &name, wxWindow* window)
{
	wxTreeMultiItem result((TreeMultiItemBase *)&_root);
	result = AppendNode(result, caption, name, window);
...
wxTreeMultiItem wxTreeMultiCtrl::AppendNode(const wxTreeMultiItem &item, const wxString &caption, const wxString &name, wxWindow *window )
{
	// add window only if this node (parent) is valid, and a node
	...
	TreeMultiItemNode *node = new TreeMultiItemNode(parent, caption, name, window );
	parent->AddNode(node);
...
void wxTreeMultiCtrl::DrawNode(TreeMultiItemBase *b, wxDC &dc)
{
	...
	if(b->IsVisible())
	{
		...
		if(b->IsTreeMultiItemNode())
		{
            ...
            wxWindow* rootwindow = n->GetWindow();

            int xx,yy;

            if (rootwindow) {
                if (m_dy_scrolled==0) {
                    CalcScrolledPosition( n->GetX(), n->GetY(), &xx, &yy);
                } else {
                    yy = n->GetY();
                    xx = n->GetX();
                }
                if (m_dy_scrolled==0) {
                    rootwindow->Reparent(this);
                    rootwindow->SetSize( xx + n->GetCaption().Len()*5, yy, rootwindow->GetSize().x, rootwindow->GetSize().y );
                }
                rootwindow->Refresh();
            }
            ...
            if (m_dy_scrolled==0) {
                ...
                yy = n->GetY() + m_dy_scrolled;
                xx = n->GetX();
            }
			dc.DrawText(n->GetCaption(), xx, yy );

			// draw the bitmap for the state
			if(n->IsExpanded())
				dc.DrawBitmap(*_expandBmp, bmpOffsetX, yy + _iconDeltaY, true);
			else
				dc.DrawBitmap(*_collBmp, bmpOffsetX, yy + _iconDeltaY, true);
...
// adjust behaviour for Linux (single click = always fold)
    ...
    //wxMessageBox(_T("You clicked in:"));
}
...
TreeMultiItemBase *wxTreeMultiCtrl::FindNodeByName(TreeMultiItemBase *b,  const wxTreeMultiItem &item) {

    if ( b->GetName() == item.GetItem()->GetName() ) {
        return b;
    }
    else
	{
		...
		if(n)
		{
			TreeMultiItemBase *bb = 0;
			for(int i = 0; i < n->GetNodeCount() && !bb; i++)
				bb = FindNodeByName(n->GetNode(i), item );
...
int wxTreeMultiCtrl::CalculateNodeDimensions(TreeMultiItemBase *b, int currentY, int level)
{
	...
	if(b->IsVisible())
	{
		...
		if(b->IsTreeMultiItemNode())
		{
			...
			//atencion!!!! esto es para poner todos en la misma linea, agregaremos una funcion para ello.
			//
...
void wxTreeMultiCtrl::AdjustScrollbars(int x, int y)
{
    ...
    if ( m_bShowHorizontal && m_bShowVertical) {
    ...
    } else if ( !m_bShowHorizontal && m_bShowVertical) {
        SetScrollbars( 0, WXTMC_PIXELS_PER_UNIT, 0,
                y/WXTMC_PIXELS_PER_UNIT, 0, y_pos, true );
    } else if ( m_bShowHorizontal && !m_bShowVertical) {
        /*SetScrollbars( WXTMC_PIXELS_PER_UNIT, 0, x/WXTMC_PIXELS_PER_UNIT,
                0, x_pos, 0, true );*/
        //SetVirtualSize(  );
    } else if ( !m_bShowHorizontal && !m_bShowVertical) {
...
void wxTreeMultiCtrl::OnScroll( wxScrollWinEvent& event ) {
    ...
    if (m_pSecondTargetWindow) {
        ...
        this->GetScrollPixelsPerUnit(&xpu,&ypu);
        this->GetVirtualSize(&vx,&vy);

        if (wxHORIZONTAL == event.GetOrientation() ) {
            //x = event.GetPosition();
            //m_pSecondTargetWindow->SetScrollRate( xpu );
        } else {
            y = event.GetPosition();
            int xpu2,ypu2;
            int vx2,vy2;
            //m_pSecondTargetWindow->GetScrollPixelsPerUnit(&xpu2,&ypu2);
            //m_pSecondTargetWindow->GetVirtualSize(&vx2,&vy2);
            //m_pSecondTargetWindow->SetVirtualSize(vx2,vy);
...
void wxTreeMultiCtrl::ScrollWindowToPos( int x, int y ) {

    if (y>-1) {
        ScrollWindow( 0, -y-m_dy_scrolled );
        m_dy_scrolled = -y;
    }
