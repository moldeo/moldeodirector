...
bool wxPinDropTarget::OnDropText(wxCoord x, wxCoord y, const wxString& str) {
...
class wxMoldeoPinDataObject : public wxDataObject {
...
DEFINE_EVENT_TYPE( wxEVT_MOLDEOPIN )
DEFINE_EVENT_TYPE( wxEVT_MOLDEOPIN_BEGINDRAGLEFT)
DEFINE_EVENT_TYPE( wxEVT_MOLDEOPIN_DRAGLEFT)
DEFINE_EVENT_TYPE( wxEVT_MOLDEOPIN_ENDDRAGLEFT)

const long wxMoldeoPinCtrl::MOLDEOPINTIMER_ID = 1;

BEGIN_EVENT_TABLE( wxMoldeoPinCtrl, wxControl )
    EVT_TIMER( wxMoldeoPinCtrl::MOLDEOPINTIMER_ID, wxMoldeoPinCtrl::OnTimer)
    EVT_PAINT( wxMoldeoPinCtrl::OnPaint )
    EVT_MOUSE_EVENTS( wxMoldeoPinCtrl::OnMouseEvent )
    EVT_MOUSE_CAPTURE_LOST( wxMoldeoPinCtrl::OnCaptureLostEvent )
    EVT_MOUSE_CAPTURE_CHANGED( wxMoldeoPinCtrl::OnCaptureChangedEvent )
END_EVENT_TABLE()
...
wxMoldeoPinCtrl::~wxMoldeoPinCtrl() {
...
void
wxMoldeoPinCtrl::OnTimer( wxTimerEvent &event) {
...
void
wxMoldeoPinCtrl::OnPaint(wxPaintEvent& event) {
...
void wxMoldeoPinCtrl::SetRange( int min, int max ) {
    if ( min >= max ) {
        wxMessageBox(wxT("wxMoldeoPinCtrl error : min is inferior or equal to max"));
    }
...
void wxMoldeoPinCtrl::ChangeValue( int value ) {
...
void wxMoldeoPinCtrl::OnMouseEvent( wxMouseEvent & event ) {

    //wxMessageBox("wxMoldeoPinCtrl::Mouse event");
...
/*
                ...
                wxTextDataObject dobject;
                dobject.SetText("Drag an drop text");
                dragSource.SetData( dobject );
...
*/
            ...
            wxCommandEvent event( wxEVT_MOLDEOPIN_DRAGLEFT, m_windowId );
            event.SetClientData( NULL );
            event.SetEventObject( this );
            GetParent()->GetParent()->AddPendingEvent( event );
            */
...
/*
    ...
    /*if (!event.Dragging()) {
        if (m_bStartDragging==true) {
            m_bStartDragging = false;
            wxCommandEvent event( wxEVT_MOLDEOPIN_ENDDRAGLEFT, m_windowId );
            event.SetClientData( NULL );
            event.SetEventObject( this );
            GetParent()->GetParent()->AddPendingEvent( event );
...
void wxMoldeoPinCtrl::OnCaptureLostEvent( wxMouseCaptureLostEvent& event ) {

    m_bStartDragging = false;

}

void wxMoldeoPinCtrl::OnCaptureChangedEvent( wxMouseCaptureChangedEvent& event ) {
